...
def generate_activity_report(beacon_key: str, start_date=None, end_date=None):
    ...
    try:
        rows = conn.execute(
            "SELECT type, event_time, created_at, distance, device_ident, beacon_id, beacon_name FROM notifications "
            "WHERE (beacon_id = %s OR beacon_name = %s) AND type IN ('in','left','still_in','still_out','missing','found')"
            + (" AND event_time >= %s" if start_date else "")
            + (" AND event_time <= %s" if end_date else "")
            + " ORDER BY id ASC",
            tuple([beacon_key, beacon_key] + ([f"{start_date} 00:00:00"] if start_date else []) + ([f"{end_date} 23:59:59"] if end_date else [])),
        ).fetchall()
    finally:
        conn.close()
...
def generate_device_activity_report(device_ident: str, start_date=None, end_date=None):
    """Generate a *device* activity PDF.

    Format required by William:
    - If the device has multiple beacons, list each beacon as a bullet item.
    - Under each beacon, list that beacon's events (IN/LEFT/STILL/MISSING/FOUND).
    """
    if not device_ident:
        return None

    # Query all notification events for this device within date range
    conn = get_db()
    try:
        rows = conn.execute(
            "SELECT beacon_id, beacon_name, type, event_time, created_at, distance "
            "FROM notifications "
            "WHERE device_ident = %s "
            "AND type IN ('in','left','still_in','still_out','missing','found') "
            + ("AND event_time >= %s " if start_date else "")
            + ("AND event_time <= %s " if end_date else "")
            + "ORDER BY id ASC",
            tuple(
    ...
    # Group by beacon
    grouped = {}
    for bid, bname, typ, event_time, created_at2, distance in rows:
        key = bid or bname or "Unknown"
        label = bname or bid or "Unknown"
        grouped.setdefault(key, {"label": label, "events": []})["events"].append(
            {
                "type": (typ or "").upper(),
                "event_time": (event_time or "").replace("T", " "),
                "logged_time": (created_at2 or "").replace("T", " "),
                "distance": distance,
            }
        )
    ...
    # Render: beacon bullet list + each beacon's events
    c.setFont("Helvetica-Bold", 11)
    c.drawString(margin, y, "Beacons")
    y -= 14

    c.setFont("Helvetica", 10)
    for _k, pack in grouped.items():
        if y < 80:
            c.showPage(); y = h - margin
        c.drawString(margin, y, f"• {pack['label']}")
        y -= 12
        c.setFont("Helvetica", 9)
        for ev in pack["events"]:
            if y < 70:
                c.showPage(); y = h - margin
                c.setFont("Helvetica", 9)
            dist_txt = f" | {ev['distance']:.2f}m" if ev["distance"] is not None else ""
            c.drawString(margin + 18, y, f"- {ev['event_time']}  {ev['type']}  (logged {ev['logged_time']}){dist_txt}")
            y -= 11
        y -= 6
        c.setFont("Helvetica", 10)
    ...
    total_events = sum(len(v["events"]) for v in grouped.values())
    summary = f"{total_events} events (device)"
    conn = get_db()
    try:
        conn.execute(
            "INSERT INTO activity_reports (beacon_name, created_at, summary, pdf_path) VALUES (%s,%s,%s,%s)",
