...
// Backend notifications polling (still_in/out, missing/found, etc.)
let lastNotifSinceIso = null;
...
lastBeaconsAgg = aggBeacons;
    ...
    // Pull newly stored notifications from backend (includes still/missing/found generated by evaluator)
...
async function pollBackendNotifications() {
  try {
    if (!lastNotifSinceIso) {
      // On first run, grab last 2 minutes so UI isn't empty after refresh.
      lastNotifSinceIso = new Date(Date.now() - 2 * 60 * 1000).toISOString();
    }

    const url = `/api/notifications/recent?since=${encodeURIComponent(lastNotifSinceIso)}`;
    const r = await fetch(url);
    if (!r.ok) return;
    const data = await r.json();
    const items = (data && data.items) ? data.items : [];

    // Keep ascending order from backend so toasts show in correct sequence.
    for (const n of items) {
      if (n && n.id != null && seenNotificationIds.has(n.id)) continue;
      if (n && n.id != null) seenNotificationIds.add(n.id);
      ...
      const msg = {
        ...
        time: n.event_time || '-',
        distance: n.distance,
        logged_time: n.created_at || null,
        beacon_id: n.beacon_id || null,
        device_ident: n.device_ident || null,
      };
      ...
      if (n.created_at) {
        lastNotifSinceIso = n.created_at;
      }
    }
  } catch (err) {
    console.error('pollBackendNotifications failed', err);
  }
}
...
function addNotification(type, beaconName, eventTime, distance, options) {
  ...
  msg.logged_time = new Date().toISOString();
...
function renderNotificationsList() {
  ...
  notifications.slice().reverse().forEach(n => {
    ...
    const logged = n.logged_time ? ` <span class="logged">• Logged: ${n.logged_time}</span>` : '';
    item.innerHTML = `
      <span class="time">${n.time}</span>${logged}
