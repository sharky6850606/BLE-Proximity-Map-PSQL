...
from services.notifications_service import save_notification as persist_notification
...
# Evaluator generates still_in/still_out and beacon missing/found events.
start_evaluator_thread(latest_messages, interval_seconds=60)
...
def api_save_notification():
    """Receive notifications from the frontend and persist them.
    ...
    The frontend sends:
      ...
      - distance
      - beacon_id (optional)
      - device_ident (optional)
    """
    data = request.get_json(silent=True) or {}
    from services.notifications_service import save_notification as _save
...
def api_notifications_recent():
    """Return notifications created after `since` (ISO string).

    Used by the map UI to show server-side evaluator events (still/missing/found)
    as live popups.
    """
    since = (request.args.get("since") or "").strip()
    # If not provided, return only a small recent window.
    if not since:
        # 5 minutes
        since = (datetime.utcnow().replace(tzinfo=timezone.utc) - timedelta(minutes=5)).isoformat()

    conn = get_db()
    try:
        if DB_MODE == "postgres":
            rows = conn.execute(
                "SELECT created_at, type, beacon_name, beacon_id, device_ident, event_time, distance FROM notifications WHERE created_at > %s ORDER BY created_at ASC LIMIT 200",
                (since,),
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT created_at, type, beacon_name, beacon_id, device_ident, event_time, distance FROM notifications WHERE created_at > ? ORDER BY created_at ASC LIMIT 200",
                (since,),
            ).fetchall()
    finally:
        conn.close()
    ...
    out = []
    for r in rows:
        created_at, ntype, name, beacon_id, device_ident, event_time, distance = r
        out.append(
            {
