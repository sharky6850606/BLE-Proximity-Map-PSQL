...
def init_db():
    ...
    try:
        if conn.backend == "postgres":
            ...
            # stable identifiers for reports + dedupe
            ...
            # created_at = when the system logged/stored the notification
            ...
            # prevent duplicate spam inserts
            cur.execute("""
                CREATE UNIQUE INDEX IF NOT EXISTS ux_notifications_dedupe
                ON notifications (type, device_ident, beacon_id, event_time)
            """)
        ...
        try:
            cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_notifications_dedupe ON notifications (type, device_ident, beacon_id, event_time)")
        except Exception:
            pass
        cur.execute("""
            CREATE TABLE IF NOT EXISTS uptime_logs (
